...
ORDER BY
  res5."Infinite Lead Score" DESC;
    """
    logger.info(f"GETTING BUYER LEAD SCORE - {buyer_email}")
...
def get_buyer_fub_stage(buyer_email):
    ...
    logger.info(f"GETTING BUYER FUB STAGE - {buyer_email}")
...
def get_buyer_profile(access_level_key: str = None, profile_ekey: str = None, profile_ikey: str = None) -> dict | None:
    ...
    if buyer_data:
        ...
        if check_if_viewer_is_admin(viewer_email):
            ...
            buyer_profile["show_contacts"] = sql_m_check_if_rca_signed(viewer_email, buyer_email)
